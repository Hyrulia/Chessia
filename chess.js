...
var MyPieces = {
		init: function() {
			...
			divWhite.innerHTML = '<div id="whitestats"><table> \
			<tr><td id="whitepawns">0</td></tr> \
			<tr><td id="whiterooks">0</td></tr> \
			<tr><td id="whitebishops">0</td></tr> \
			<tr><td id="whiteknights">0</td></tr> \
			<tr><td id="whitequeen">0</td></tr> \
			</table></div>';
			
			divBlack.innerHTML = '<div id="blackstats"><table> \
			<tr><td id="blackpawns">0</td></tr> \
			<tr><td id="blackrooks">0</td></tr> \
			<tr><td id="blackbishops">0</td></tr> \
			<tr><td id="blackknights">0</td></tr> \
			<tr><td id="blackqueen">0</td></tr> \
			</table></div>';
			
			$('#chessDesk').append(divWhite);
			$('#chessDesk').append(divBlack);
		},
		run: function() {
			this.pieces = [];
			
			for (var idx in this.white)
				MyPieces.pieces[this.white[idx]] = 0;
			
			for (var idx in this.black)
				MyPieces.pieces[this.black[idx]] = 0;
			
			$('span[data-piece]').each(function(i) {
				MyPieces.pieces[this.dataset['piece']]++;
			});
		...
		whiteToHTML: function() {
			
			$('#whiterooks').text(2 - this.pieces[this.white.rooks]);
			$('#whitebishops').text(2 - this.pieces[this.white.bishops]);
			$('#whiteknights').text(2 - this.pieces[this.white.knights]);
			if(this.maxQueen.white < this.pieces[this.white.queen])
				this.maxQueen.white = this.pieces[this.white.queen];
			$('#whitepawns').text(8 - this.pieces[this.white.pawns] - (this.maxQueen.white - 1));
			$('#whitequeen').text(this.maxQueen.white - this.pieces[this.white.queen]);
			
		}, 
		blackToHTML: function() {
			
			$('#blackrooks').text(2 - this.pieces[this.black.rooks]);
			$('#blackbishops').text(2 - this.pieces[this.black.bishops]);
			$('#blackknights').text(2 - this.pieces[this.black.knights]);
			if(this.maxQueen.black < this.pieces[this.black.queen])
				this.maxQueen.black = this.pieces[this.black.queen];
			$('#blackpawns').text(8 - this.pieces[this.black.pawns] - (this.maxQueen.black - 1));
			$('#blackqueen').text(this.maxQueen.black - this.pieces[this.black.queen]);
...
var chess = (function() {
	...
	function quakeShake(x) {
		
		if(x == 0) { //black
			clearInterval(interval)
			interval = setInterval(function() {$('#blackking').effect('shake', {distance: 5});}, 3000);
		} else { //white
			clearInterval(interval)
	...
	function getInCheckPieces() {
		...
		for (var iExamSq = 21; iExamSq < 99; iExamSq += iExamSq % 10 < 8 ? 1 : 3) {
			...
			if ((!bCheck || (iExamPc & 7) === 2) && iExamPc > 0 && (iExamPc & 8 ^ 8) === flagWhoMoved && isThreatened(iExamX, iExamY, flagWhoMoved)) { etc.aThreats.push(iExamSq); checkmates(); }
		}
		if (bNoMoreMoves) {	
			if (bCheck) {
				var sWinner = flagWhoMoved ? 2 : 1;
				oGameInfo.Result = flagWhoMoved ? "0-1" : "1-0";
				Popup.showWin(sWinner);
			...
			var sWinner = oGameInfo.Result.valueOf() === "1-0" ? 1 : 2;
			Popup.showWin(sWinner);
			bGameNotOver = false;
	...
	*/
	...
	function writeFlatPieces() {
		...
		for (var iCell = 0; iCell < 64; iCell++) {
			...
			sSqrContent = etc.aBoard[nSquareId]; oSquareCell.innerHTML = sSqrContent === 0 ? "" : '<span data-piece="'+ '171216151413231822212019'.substr((((sSqrContent & 15) * 3 + (sSqrContent & 7)) >> 1) - 2, 2) 
			+'"><img width="48px" src="img/' + '171216151413231822212019'.substr((((sSqrContent & 15) * 3 + (sSqrContent & 7)) >> 1) - 2, 2) + '.png" /><\/span>';
...
// DOM private APIs
	...
	function getSqFnc() {
		if(!Popup.isShown) {
			...
			mapper.i = parseInt(getId / 10);
			mapper.j = parseInt(getId % 10);
			alert(mapper.i + '  ' + mapper.j)
	...
	function showDialog(value) {
		...
		div.title = "chessia";
		clearInterval(interval);
		div.innerHTML = '<div class="winorlose">' + value + '</div>'
		$(div).dialog({
			height: 240,
			width: 337, 
			modal: false,
	...
	function checkmates() {
		...
		if(bCheck) {
			for(var i = 0; i < kings.length; i++)
				if($.inArray(kings[i], etc.aThreats) != -1) {
					setTimeout(function() {
						$('#flatSq' + kings[i] + ' > span').twinkle(options);
					}, 100);
					return;
				}
		}
